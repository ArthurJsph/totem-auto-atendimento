    ...
    getAllProducts()
      .then((data) => {
        ...
        const todosProdutos = [
          ...data,
          ...produtos.filter(
            (p) => !data.some((apiProd: any) => apiProd.id === p.id)
          ),
        ];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          ...
          {products.map((product: any) => (
            <div key={product.id} className="border rounded p-4 shadow hover:shadow-lg transition">
              ...
              />
              <h2 className="text-xl font-semibold mb-2">{product.nome || product.name}</h2>
              <p className="text-gray-600 mb-2">{product.descricao || product.description}</p>
              <p className="font-bold text-lg">R$ {(product.preco || product.price).toFixed(2)}</p>
            </div>
          ))}
        </div>
      )}
